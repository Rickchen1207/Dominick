...
oj <- read.csv("oj.csv") 
head(oj)
str(oj$brand) # chr or factor? if chr, need to turn it into factor
oj$brand<-as.factor(oj$brand)
str(oj$brand) # now factor
levels(oj$brand)
table(oj$brand)

avg_price<-oj %>% group_by(brand) %>% summarise(lp_mean=mean(log(price)), lpmedian=median(log(price)))

levels(oj$brand) # check the levels of our factor variable
...
# (1st model) simple regression 
reg_simple = glm(log(sales) ~ log(price), data=oj)

## use the fitted model
summary(reg_simple) ## coef, tests, fit
coef(reg_simple) ## just coefficients

## simple regression (with brand dummies)
